...
MODEL = "/home/muhdirfanrosdin/resAI/MCTA4363-Resilient-AI/models/gemma4-E4B-Q6_K.gguf"
...
def run_inference(image, question, idx=0):
    ...
    cmd = [
        ...
        "--mmproj", MMPROJ,
        "--image", img_path,
        "-p", f"<|turn>system\nRespond with only the answer, no reasoning.\n<turn|><|turn>user\nQuestion: {question}<turn|><|turn>model\n",
        "-n", "30",
        "--gpu-layers", "99",
        "--jinja",
    ]
...
print("\n--- EXPERIMENT 006: Q6_K GGUF (llama.cpp) ---")
...
report = {
    "model": "gemma-4-E4B-it Q6_K GGUF llama.cpp",
...
with open("006_results.json", "w") as f:
...
print(f"{'='*45}")
print("Saved to 006_results.json")